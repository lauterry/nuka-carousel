...
var Carousel = (0, _createReactClass.default)({
  ...
  formatChildren: function formatChildren(children) {
    ...
    return _react.default.Children.map(children, function (child, index) {
      ...
      var isLeftslide = self.state.currentSlide === index + 1 || self.state.currentSlide === 0 && index === children.length - 1;
      var isRightSlide = self.state.currentSlide === index - 1 || self.state.currentSlide === children.length - 1 && index === 0;
